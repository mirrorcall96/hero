...
const Main =(props)=> {
    ...
    for (let i = 0; i < Math.min(props.villains.length,9); i+=3) {
        cards.push(<Col xs={2}>{props.villains.filter((a,j) => j>=i && j<i+3).map(a=><StyledCard className="villian" key={a} onClick={()=>props.attackOnVillian(a)} ><StyledCard.Img src={vlogos[a % vlogos.length]} /><StyledCard.Title><center>{a}</center></StyledCard.Title></StyledCard>)}</Col>)
    }
